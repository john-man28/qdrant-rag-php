...
it('uses the hybrid nearest query path and preserves response shape and session state', function () {
    ...
    $agent = new CatalogHybridAgentHarness(
        config: makeAgentConfig(hybridEnabled: true),
...
it('keeps dense-only nearest search unchanged when hybrid is disabled', function () {
    ...
    $agent = new CatalogHybridAgentHarness(
        config: makeAgentConfig(hybridEnabled: false),
        vectorEncodings: new CatalogVectorEncodingOrchestrator(
...
it('fails fast when hybrid search is enabled without a late interaction model config', function () {
    $transport = new CatalogHybridCaptureTransport(
        fn (HttpRequest $request): HttpResponse => qdrantJsonResponse(['result' => ['exists' => true]]),
    );
...
function makeAgentConfig(bool $hybridEnabled): CatalogAgentConfig
{
